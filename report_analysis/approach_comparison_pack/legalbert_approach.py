"""LegalBERT exact comparison approach."""
...
def build_approach(force: bool = False) -> HuggingFaceMeanPoolingApproach:
    """Create the LegalBERT approach."""
    local_dir = PROJECT_ROOT / "models" / "hf" / "nlpaueb--legal-bert-base-uncased"
...
def main() -> None:
    """Run LegalBERT as a standalone comparable experiment."""
    args = parse_common_args("Evaluate LegalBERT mean-pooled dense retrieval on the shared benchmark.")
    records = load_case_records(args.max_docs)
    queries = load_queries(args.eval_path, args.max_queries)
    output = args.output or RESULTS_DIR / "legalbert_results.csv"
